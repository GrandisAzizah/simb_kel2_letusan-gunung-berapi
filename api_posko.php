...
switch ($action) {
    ...
    case 'tambah_logistik':
        ...
        break;
    case 'get_logistik': // BARU
        get_single_data($koneksi, 'logistik', 'id_logistik', $_GET['id']);
        break;
    case 'update_logistik': // BARU
        update_logistik($koneksi, $_POST);
        break;
    case 'hapus_logistik':
    ...
    case 'tambah_stok':
        ...
        break;
    case 'get_stok': // BARU
        get_single_data($koneksi, 'stok', 'id_stok', $_GET['id']);
        break;
    case 'update_stok': // BARU
        update_stok($koneksi, $_POST);
        break;
    case 'hapus_stok':
...
function load_all_data($k) {
    ...
    $sqlStok = "SELECT s.id_stok, s.id_posko, s.id_logistik, p.nama_posko, l.nama_barang, l.satuan, s.jumlah_stok, s.update_terakhir 
                FROM stok s
...
function update_logistik($k, $data) { // FUNGSI BARU
    if (!isset($data['id_logistik_edit'])) {
        echo json_encode(['status' => 'error', 'message' => 'Data edit tidak lengkap']);
        return;
    }

    $stmt = $k->prepare("UPDATE logistik SET nama_barang = ?, satuan = ? WHERE id_logistik = ?");
    $stmt->bind_param("ssi", 
        $data['nama_barang_edit'], 
        $data['satuan_edit'], 
        $data['id_logistik_edit']
    );
    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'message' => 'Logistik berhasil diperbarui']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Gagal memperbarui logistik: ' . $stmt->error]);
    }
    $stmt->close();
}
...
function update_stok($k, $data) { // FUNGSI BARU
    if (!isset($data['id_stok_edit'])) {
        echo json_encode(['status' => 'error', 'message' => 'Data edit tidak lengkap']);
        return;
    }
    ...
    $stmt = $k->prepare("UPDATE stok SET id_posko = ?, id_logistik = ?, jumlah_stok = ?, update_terakhir = ? WHERE id_stok = ?");
    $stmt->bind_param("iiisi", 
        $data['id_posko_edit'], 
        $data['id_logistik_edit'], 
        $data['jumlah_stok_edit'],
        $now,
        $data['id_stok_edit']
    );
    if ($stmt->execute()) {
        ...
        echo json_encode(['status' => 'error', 'message' => 'Gagal memperbarui stok: ' . $stmt->error]);
    }
    $stmt->close();
}
