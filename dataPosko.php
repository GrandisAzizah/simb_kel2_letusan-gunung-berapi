...
$data = query("SELECT nama_posko, alamat_posko, kapasitas_maksimal, penanggung_jawab FROM posko_pengungsian order by id_posko DESC");
...
<head>
    ...
    <title>Data Posko & Logistik</title>
...
<body>
    ...
    <!-- Judul -->
    <h2 class="judul-poppins text-center mt-4 mb-4">Data Posko & Logistik</h2>

    <!--search bar  -->
    <div class="container-fluid mb-3 mt-3 justify-content-center d-flex">
        <form action="" method="POST" class="d-flex" role="search">
            <input class="form-control me-2" name="keyword" type="text" placeholder="Cari Posko Terdekat" aria-label="Search" autofocus autocomplete="off" />
            <button class="btn btn-outline" type="submit" name="cari">Search</button>
        </form>
    </div>
        ...
        <table class="table table-bordered">
            ...
            <?php foreach ($data as $row) : ?>
                <tr>
                    ...
                    <td><?= $row["kapasitas_maksimal"]; ?></td>
                    <td><?= $row["alamat_posko"]; ?></td>
                    <td><a href="https://www.google.com/maps/search/<?= urlencode($row['alamat_posko']); ?>" target="_blank">Lihat di Google Maps</a></td>
                    <td>
                        <a href="https://www.google.com/maps/dir//<?= urlencode($row['alamat_posko']); ?>"
                            target="_blank"
